...
async function startXeonBotInc() {
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Felix-MD Ofc','Safari','1.0.0'],
        auth: state
    })
    ...
    let halal = await XeonBotInc.sendContact(callerId, global.owner)
...
title: `${ownername}`,
body: `Jangan Lupa Baca deskripsi dek!`,
mediaType:2,
thumbnail: XeonWlcm,
sourceUrl: `${websitex}`,
mediaUrl: `${websitex}`
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   └───────────────┈ ⳹`
let buttons = [
{buttonId: `wkwkwk`, buttonText: {displayText: 'Byee'}, type: 1}
]
let buttonMessage = {
document: fs.readFileSync('./Media/theme/cheems.xlsx'),
mimetype: docs,
...
XeonBotInc.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    * Send Poll
    */
    XeonBotInc.sendPoll = async (jid, name = '', optiPoll, options = {}) => {
        ...
        options: optiPoll.map(btn => ({
                ...
                return XeonBotInc.relayMessage(jid, { pollCreationMessage: pollMessage }, { ...options });
                }
